...
+ select的作用及使用
  ...
  3、select有比较多的限制,其中最大的一条限制就是每个case语句里面必须是一个IO操作。
  4、break只能跳出⼀个case分⽀不能跳出select外⾯的for
  5、传统的方法在遍历管道时，如果不关闭会阻塞而导致 deadlock，可以使用 select 方式可以解决
  select {
    case <-chan1:
    ...
    default:
        //如果上面都没有成功,则进入default处理流程.如果没有default语句,那么select语句将被阻塞,直到至少有一个通信可以进行下去.
  }
  ```
...
结果：
    ...
    3 -> 4
解释：
    1、for循环过程中只是对每个val进行重新开辟空间，不是地址引用，当使用for循环取每个每个元素所指向的地址都是最后一个元素的地址;
    2、切片在内存中是一个有序集合，可以想象成一个链表，所以只需要知道头结点就知道整个切片，go语言的切片所指向的地址就是最后一个元素的地址，
```
